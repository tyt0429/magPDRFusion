function [location,location2,location3,location4,location5,location6, indicator1, indicator2] = Matching(magnetmap,magnetseries,gridrelationship,X,Y,Gridsize,coordinate)
%%展示地磁基準網格圖
    ...
    for x=1:X
        for y=1:Y
            ...
            if((mapindex(:,1)<=X)&(mapindex(:,2)<=Y))
                ...
                %dtw
                [dist,ix,iy]= dtw(submagnetseries(1,:),magnetmap2(1,:));
                distv(slide) = dist;
                series_l(slide) = length(ix);
...
%     indicator1 = distv(index1(1))/series_l(index1(1));
    indicator2 = distv(index1(1))/length(submagnetseries(1,:));
%     
