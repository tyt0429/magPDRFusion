...
for k=1:1
    ...
    i=size(G,1);
    % a:vertical component b:horizontal componeent c:magnitude
    % V/H:vector
    for j=1:i
    ...
    D=[a;b;c];
    F=smoothdata(D','movmedian',0.1/(time_interval)*5);%移動平均
    F=F';
    %     figure('name','MAG');
    %     plot(D(1,:));
    %     hold on;
...
for i=2:size(locsfin,1)
    cdistance(i)=cdistance(i-1)+step(i); % accumulated distance
end
...
for i = 2:1:size(mag,1)+1
    ...
    if((i-update)==110)
        ...
        sublocsfin=locsfin(1:nnindex(1));%步的筆數次序列   
        start=locsfin(size(sublocsfin,1));%start=最接近i的一步
        ...
        %更新後的點繼續更新gyro的azimuth
        [g_d_az]=gyroheading(gyro,time_interval,start,g_d_az);
        %更新時間點
        update=i;
        updatelocsfin=nnindex(1); %更新的步數
